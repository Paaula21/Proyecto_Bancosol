...
<div class="app-container">
    ...
    <div class="right-content">
        ...
        <div class="main-layout">
            <main style="padding: 24px; box-sizing: border-box; width: 85%; margin: 20px auto; background-color: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); overflow-y: auto;">
                        ...
                        <table class="data-table" style="width: 100%; border-collapse: collapse; text-align: left;">
                            ...
                            <%
                                for (int i = 0; i < diasSemana.length; i++) {
                                    String nombreSelectManana = "asignacion_manana_" + idsDia[i];
                                    String nombreSelectTarde  = "asignacion_tarde_"  + idsDia[i];
                            %>
                            <tr style="border-bottom: 1px solid #e5e7eb;">
                                <td style="padding: 12px; font-weight: 600; color: #111827;"><%= diasSemana[i] %></td>
                                    ...
                                    <select name="<%= nombreSelectManana %>" style="width: 100%; padding: 8px; border-radius: 6px; border: 1px solid #d1d5db; background-color: #f9fafb;">
                                        ...
                                        <% if (voluntariosManana != null) {
                                            for (VistaVoluntarioDTO v : voluntariosManana) {
                                                boolean seleccionado = asignaciones != null && String.valueOf(v.getIdVoluntario()).equals(asignaciones.get(nombreSelectManana));
                                        %>
                                        <option value="<%= v.getIdVoluntario() %>" <%= seleccionado ? "selected" : "" %>><%= v.getNombreCompleto() %></option>
                                        <%  }
                                        } %>
                                    </select>
                                </td>
                                    ...
                                    <select name="<%= nombreSelectTarde %>" style="width: 100%; padding: 8px; border-radius: 6px; border: 1px solid #d1d5db; background-color: #f9fafb;">
                                        ...
                                        <% if (voluntariosTarde != null) {
                                            for (VistaVoluntarioDTO v : voluntariosTarde) {
                                                boolean seleccionado = asignaciones != null && String.valueOf(v.getIdVoluntario()).equals(asignaciones.get(nombreSelectTarde));
                                        %>
                                        <option value="<%= v.getIdVoluntario() %>" <%= seleccionado ? "selected" : "" %>><%= v.getNombreCompleto() %></option>
                                        <%  }
                                        } %>
